  ...
  if(moderator.scale == "continuous"){
    ...
    if(length(xintercept) > 1){
      pMain = ggplot(results, aes(x = results[, moderator], y = results[, effect])) +
        ...
        geom_vline(xintercept = xintercept, linetype = "dashed", col = "grey48") + 
        lapply(1:length(xintercept), function(s) {geom_point(aes(x = rep(xintercept[s], nrow(results)), y = rep(0, nrow(results))), col = "grey18")}) + 
        lapply(1:length(xintercept), function(s) {geom_text(aes(x = rep(xintercept[s], nrow(results)), y = rep(0, nrow(results)), label = paste0("(", round(xintercept[s], 2), ", 0)")), size = 3,  col = "grey18", hjust = 0.5, vjust = 1.5)
      })
    }
  }
